...
def parser(add_help=True):
    ...
    parser.add_argument(
        ...
        type=str2bool,
        default=False,
        help="Whether use GPU to predict. Default by False.")
    parser.add_argument(
        "--enable_mkldnn",
        type=str2bool,
        default=True,
...
class ImageWriter(object):
    ...
    def write(self, image, file_name):
        ...
        cv2.imwrite(path  , image )   # , cv2.cvtColor(image, cv2.COLOR_RGB2BGR))
...
class Detector(BasePredictor):
    def __init__(self, det_config, predictor_config, filter_boxes_by_aspect_ratio=True):
        super().__init__(predictor_config)
        self.det_config = det_config
        self.target_size = self.det_config["target_size"]
        self.thresh = self.det_config["thresh"]
        self.filter_boxes_by_aspect_ratio = filter_boxes_by_aspect_ratio
        self.detected_dir = predictor_config["dir_output"] + "/detected"

        # Créer le dossier "detected" s'il n'existe pas
    ...
    def preprocess(self, img):
        ...
        img_shape = img.shape
        self.original_img = img  # Sauvegarder l'image originale pour le recadrage plus tard
        img_scale_x = resize_w / img_shape[1]
        img_scale_y = resize_h / img_shape[0]
        img = cv2.resize(
            img, None, None, fx=img_scale_x, fy=img_scale_y, interpolation=cv2.INTER_LINEAR)
        img = normalize_image(
            img,
            scale=1. / 255.,
    ...
    def postprocess(self, np_boxes):
        ...
        valid_boxes = (np_boxes[:, 1] > self.thresh) & (np_boxes[:, 0] > -1)
        np_boxes = np_boxes[valid_boxes, :]

        filtered_boxes = []
        for box in np_boxes:
            x_min, y_min, x_max, y_max = box[2:6]  # Supposant que les coordonnées sont [x_min, y_min, x_max, y_max]
            width = x_max - x_min
            height = y_max - y_min
            if not self.filter_boxes_by_aspect_ratio or width <= height:
                filtered_boxes.append(box)
            # Enregistrer le crop de l'image
            self.save_crop(self.original_img, x_min, y_min, x_max, y_max)

        return np.array(filtered_boxes)

    def save_crop(self, img, x_min, y_min, x_max, y_max):
        ...
        # Recadrer l'image avec des vérifications de limites
        crop_img = img[max(0, y_min):min(img.shape[0], y_max), max(0, x_min):min(img.shape[1], x_max)]
        
        # Vérifier si la largeur du crop est supérieure à la hauteur
        crop_height, crop_width = crop_img.shape[:2]
        if crop_width > crop_height:
            return  # Ne pas sauvegarder si la largeur est supérieure à la hauteur
        ...
        idx = 1
        while True:
            crop_filename = os.path.join(self.detected_dir, f"{timestamp}_{idx}.png")
            if not os.path.exists(crop_filename):
                break
            idx += 1

        # Sauvegarder le crop de l'image
...
class InsightFace(object):
    ...
    def init_det(self, args):
        ...
        det_predictor_config["dir_output"] = args.output
